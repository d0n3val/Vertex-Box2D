...
bool ModuleRender::Blit(SDL_Texture* texture, int x, int y, SDL_Rect* section, float speed, double angle, int pivot_x, int pivot_y )
{
	bool ret = true;
	SDL_Rect rect;
	rect.x = (int) ((camera.x * speed) + x * zoom);
	rect.y = (int) ((camera.y * speed) + y * zoom);
...
bool ModuleRender::DrawQuad(const SDL_Rect& rect, Uint8 r, Uint8 g, Uint8 b, Uint8 a, bool filled, bool use_camera)
{
	...
	if(use_camera)
	{
		...
		rec.w = (int)((float)rect.w * zoom);
		rec.h = (int)((float)rect.h * zoom);
	}
...
bool ModuleRender::DrawLine(int x1, int y1, int x2, int y2, Uint8 r, Uint8 g, Uint8 b, Uint8 a, bool use_camera)
{
	...
	if(use_camera)
		result = SDL_RenderDrawLine(renderer, camera.x + x1 * zoom, camera.y + y1 * zoom, camera.x + x2 * zoom, camera.y + y2 * zoom);
	else
		result = SDL_RenderDrawLine(renderer, x1 * zoom, y1 * zoom, x2 * zoom, y2 * zoom);
